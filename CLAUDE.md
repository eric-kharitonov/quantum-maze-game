...
**Backend (`server.py`)** — a single Flask route `POST /collapse` that accepts a JSON body with `type: "w_state"`, `type: "bell"`, `type: "nonzero"`, or `type: "chsh"`. It builds the corresponding Qiskit `QuantumCircuit`, runs it with `shots=1` on `AerSimulator`, and returns the measurement outcome. The `nonzero` type is the v0.3/v0.4.1 mechanic — `build_nonzero(k)` prepares `|ψ⟩ = (1/√(2^k − 1)) Σ_{x ≠ 0} |x⟩` directly via `qc.initialize` on a target statevector with amplitude 0 on `|00…0⟩` and `1/√(2^k − 1)` everywhere else. `sample_nonzero(k)` runs the circuit once — no rejection loop, since the all-zero outcome is forbidden by the unitary. The Bell circuit is a standard Φ⁺ state. Qiskit bitstrings are big-endian (reversed before use).
